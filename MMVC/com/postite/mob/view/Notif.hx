...
import com.postite.mob.signal.NoteSignal.NoteType;
import com.postite.mob.signal.NoteSignal.INotif;

class Notif extends Sprite implements INotif{

	var box:NoteBox;
	var message:String;
	var type:NoteType;
	var scope:DisplayObjectContainer;
	public function new(_scope:DisplayObjectContainer,_message:String,?autoKill:Bool=true,?_type:NoteType)
	{
		super();
		
		type= (_type==null)? INFO : _type; 
		trace( "new Notification" +type);
		message=_message;
		scope=_scope;
		draw();
